...
void Application::Start() {
...
#if CONFIG_PUSH_CHANNEL_ENABLED
    // ========== 初始化推送通道 ==========
    push_channel_ = std::make_unique<PushChannel>();

    // 设置推送开始播放回调：暂停唤醒词检测
    push_channel_->OnPlayStarted([this]() {
        Schedule([this]() {
            ESP_LOGI(TAG, "Push audio started, stopping wake word detection");
            wake_word_->StopDetection();
        });
    });

    // 设置推送播放结束回调：恢复唤醒词检测
    push_channel_->OnPlayFinished([this]() {
        Schedule([this]() {
            ESP_LOGI(TAG, "Push audio finished, resuming wake word detection");
            if (device_state_ == kDeviceStateIdle) {
                wake_word_->StartDetection();
            }
        });
    ...
    // 设置推送通道状态变化回调：更新显示
    push_channel_->OnStateChanged([this](PushChannelState state) {
        Schedule([this, state]() {
            auto display = Board::GetInstance().GetDisplay();
            if (state == kPushChannelConnected) {
                ESP_LOGI(TAG, "Push channel connected");
                display->ShowNotification("推送通道已连接");
            } else if (state == kPushChannelDisconnected) {
                ESP_LOGI(TAG, "Push channel disconnected");
            } else if (state == kPushChannelPlaying) {
                ESP_LOGI(TAG, "Push channel playing");
                display->SetStatus("正在播放推送...");
            }
        });
    });

    // 启动推送通道（空闲时保持连接，等待服务器推送）
    push_channel_->Start();
    // ========== 推送通道初始化完成 ==========
#endif
...
#if CONFIG_PUSH_CHANNEL_ENABLED
            // 如果推送通道正在播放，打断它
            if (push_channel_->IsPlaying()) {
                ESP_LOGI(TAG, "Wake word detected while push playing, aborting push");
                push_channel_->Abort();
            }
#endif
